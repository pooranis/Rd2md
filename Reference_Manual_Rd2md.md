...
================
  ...
  - [`Rd2md-package-package`](#rd2md-package-package)
  - [`RdTags`](#rdtags)
...
This function will parse an Rd object returning a list with each
section. The contents of each element of the list will be converted to
markdown.
...
a named list with the parts of the Rd object that will be used for
...
This function will convert an Rd element to markdown format. Note that
links are only supported within a markdown document, referenced with \#.
...
Rd2markdown(rdfile, outfile, append = FALSE, section = "#",
  subsection = "##", run.examples = FALSE, Rmd = T)
```
...
| `Rmd`          | logical. should the output be in Rmarkdown format or regular markdown? default: TRUE, rmarkdown |
...
# `Rd2md-package-package`

Reference Manual in Markdown

## Description

Functions to parse and convert Rd files to markdown format reference
manual.
...
ReferenceManual(pkg = getwd(), outdir = getwd(), front.matter = "",
  toc.matter = "<!-- toc -->", date.format = "%B %d, %Y",
  verbose = FALSE, title.level = 1, run.examples = FALSE,
  skip.topics = NULL, sepexported = FALSE, man_file = NULL)
```
...
| `man_file`     | Character. name of output file. Default Reference\_Manual\_pkgname.md                                                                                                                                    |
| `sepxported`   | Logical. Separate exported and internal objects. Exported objects should each have their own .rd file                                                                                                    |
...
Murdoch, D. (2010). [Parsing Rd
files](http://developer.r-project.org/parseRd.pdf)
...
Strip white space (spaces only) from the beginning and end of a
character.
